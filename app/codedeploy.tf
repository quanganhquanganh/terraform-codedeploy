  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    ${element(formatlist(", { \"Effect\" : \"Allow\", \"Action\" : [ \"s3:PutObject*\", \"s3:ListBucket\" ], \"Resource\" : [ \"%s/*\", \"%s\" ] }, { \"Effect\" : \"Allow\", \"Action\" : [ \"s3:ListAllMyBuckets\" ], \"Resource\" : [ \"*\" ] }", compact([var.s3_bucket_arn]), compact([var.s3_bucket_arn])), 0)}
   ]
}
EOF
